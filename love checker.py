...
def result():
    ...
    if len(b_name_var.get()) == 0 or len(g_name_var.get()) == 0:
        ...
        if len(b_name_var.get()) == 0 or len(g_name_var.get()) == 0:
            ...
            if b_name_var.get().lower() == g_name_var.get().lower():
                messagebox.showinfo('INFO', 'different names please')
            else:
                if random_result < 35:
...
check_love = ctk.CTkButton(love_frame, text='Check love', border_color='#00ff00',border_width=2, fg_color='transparent' ,hover_color='#00ff00', font=btn_font, command=result)
